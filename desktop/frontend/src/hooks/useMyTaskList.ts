...
export const useMyTaskList = () => {
  ...
  const getTasks = async (projectId?: string, statusList?: string[]) => {
    ...
    setLoading(true);
    GetTasks(projectId, statusList??[]).then((tasks) => {
      setTasks(tasks);
    }).catch((error) => {
  ...
  return { tasks, activeTask, loading, setActiveTask, getTasks, setTasks };
}